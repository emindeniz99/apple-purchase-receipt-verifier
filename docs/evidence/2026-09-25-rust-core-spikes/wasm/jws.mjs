...
if (process.argv[2] === 'web') await mod.default({ module_or_path: readFileSync('$SCRATCH/spike-wasm/pkg-web/aprv_wasm_bg.wasm') });
const { ReceiptVerifier, JwsVerifier } = mod;
const rd = (p) => new Uint8Array(readFileSync('$REPO/fixtures/' + p));
const txt = (p) => readFileSync('$REPO/fixtures/' + p, 'utf8');
