#!/bin/sh
# Round 3's parity.sh, with this round's reference and output: one artifact
# on one host, all 1,179 rows (CORPORA_LIST=fuzz for the 5,000 mutants),
# compared row by row with the native round-3 CMS baseline without the
# prescan ($SCRATCH/asn1/run/base-<corpus>.jsonl, from `parity.sh native
# base ...`). The runners are the wasm bake-off's, unchanged ($PREV/js,
# $PREV/py, $PREV/browser, $PREV/workerd, the wazero runner); only the
# output goes to this folder's results/parity.txt.
#
