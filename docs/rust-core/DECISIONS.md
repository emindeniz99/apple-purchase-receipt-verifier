...
at a pinned Apple root. The owner chose to match Java rather than
exclude MD5. The remaining risk: only a key under a pinned Apple root
can produce the signature, so an attacker would need a collision attack
against something Apple itself signs.
