...
| Legacy receipt order: CMS, creation date only, chain, signature, full parse; unreadable signed content is `INTERNAL_ERROR` (21009) | ✅ `ReceiptPayload.readCreationDate`, BouncyCastle ASN.1 | ✅ `readCreationDate`, both builds | ✅ `_read_creation_date` | ✅ `readCreationDate` | ✅ `ReceiptPayload.creation_date` | ✅ `ReceiptPayload::readCreationDate` | ✅ `ReceiptPayload.ReadCreationDate`, `System.Formats.Asn1` | ✅ `read_creation_date` | ✅ `readCreationDate` | ✅ inherits Rust; `APRV_REASON_INTERNAL_ERROR` = 12 |
| JWS typed read after the signature: a modelled claim of the wrong JSON type is `INTERNAL_ERROR`, `1.0` reads as 1 | ✅ `StrictClaims` | ✅ `readTransactionPayload`, both builds | ✅ `_read_typed_claims` | ✅ `typedClaims` | ✅ `Payload.read` | ✅ `JwsClaims::toTransaction` | ✅ `Claims.TypedString` / `Int64` | ✅ `from_claims` returns `Result` | ✅ `claimString` / `claimInteger` | ✅ inherits Rust |
