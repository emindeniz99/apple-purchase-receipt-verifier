...
const CERTIFICATES = [
  ...['AppleIncRootCertificate.cer', 'AppleRootCA-G2.cer', 'AppleRootCA-G3.cer']
    .map((name) => ({ name: `certs/${name}`, der: read(`../certs/${name}`) })),
  ...readdirSync(fileURLToPath(generatedDir)).filter((name) => name.endsWith('-root.der')).sort()
    .map((name) => ({
...
test('the fixture set covers both key types and is not empty by accident', () => {
  ...
  assert.deepEqual([...kinds].sort(), ['EC', 'RSA']);
...
for (const { name, der } of CERTIFICATES) {
  test(`${name}: SPKI converts to the same JWK a native "spki" import exports`, async () => {
    ...
    const native = await webcrypto.subtle.importKey('spki', parseCertificate(der).spki,
      importParams(jwk), true, ['verify']);
...
test('a compressed EC point is refused, not guessed at', () => {
  const cert = CERTIFICATES.map(({ der }) => parseCertificate(der))
    .find((c) => spkiToJwk(c.spki).kty === 'EC');
  ...
  spki[point] = 0x03;
  assert.throws(() => spkiToJwk(spki), (error) => {
...
test('an unknown public key algorithm is an INVALID_CERTIFICATE, not a raw throw', () => {
  ...
  const cert = CERTIFICATES.map(({ der }) => parseCertificate(der))
    .find((c) => spkiToJwk(c.spki).kty === 'RSA');
  const spki = Uint8Array.from(cert.spki);
