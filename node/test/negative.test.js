...
test('rejects a tampered payload segment', () => {
  ...
  assert.throws(
    () => verifier().verifyTransaction(`${header}.${forged}.${signature}`),
...
test('rejects a tampered receipt payload byte', () => {
  ...
  assert.throws(
    () => receiptVerifier.verify(tampered),
    (e) => e.reason === 'INVALID_SIGNATURE',
  );
});
...
test('rejects garbage receipt bytes', () => {
  ...
  assert.throws(
    () => receiptVerifier.verify(Buffer.from([1, 2, 3, 4])),
...
test('bundled Apple roots are all three published roots', () => {
  ...
  for (const roots of [appleJwsRoots(), appleReceiptRoots()]) {
    ...
    assert.ok(
      subjects.some((s) => /Apple Root CA - G2/.test(s)),
      subjects,
    );
    assert.ok(
      subjects.some((s) => /Apple Root CA - G3/.test(s)),
      subjects,
    );
