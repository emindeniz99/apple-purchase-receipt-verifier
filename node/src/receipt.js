...
import { ParseError, Tag, encodeOidContents, isOctetString, octetStringValue, parse, tbsParts } from './der.js';
import { buildAndValidatePath, normalizeRoots } from './chain.js';
...
const ATTR = {
  BUNDLE_ID: 2, APP_VERSION: 3, OPAQUE_VALUE: 4, SHA1_HASH: 5,
  CREATION_DATE: 12, IN_APP: 17, ORIGINAL_APP_VERSION: 19, EXPIRATION_DATE: 21,
};
const IAP = {
...
export class ReceiptVerifier {
   ...
   * @param {Array} options.trustedRoots pinned roots (prod: `appleReceiptRoots()`)
   * @param {string} options.bundleId bundle id the receipt must carry
   */
  constructor({ trustedRoots, bundleId }) {
   ...
   */
  verify(receipt, deviceGuid = null) {
    ...
    buildAndValidatePath(signerCert, embedded, this.#roots, at);
...
function parseCms(der) {
  ...
  try {
    if (contentInfo.tag !== Tag.SEQUENCE
      || !contentInfo.children[0].contents.equals(OID_SIGNED_DATA)
      || contentInfo.children[1].tag !== Tag.CONTEXT_0) {
    ...
    const encap = children[2];
    if (encap.children.length < 2 || encap.children[1].tag !== Tag.CONTEXT_0) {
      throw new ParseError('no encapsulated payload');
    }
    const contentNode = encap.children[1].children[0];
    ...
    let certificates = [];
    for (const child of children.slice(3, children.length - 1)) {
      if (child.tag === Tag.CONTEXT_0) {
        certificates = child.children.map((c) => c.raw);
      }
    }
    const signerInfos = children[children.length - 1];
    if (signerInfos.tag !== Tag.SET || signerInfos.children.length === 0) {
      throw new ParseError('no signer info');
    }
    const signerInfo = parseSignerInfo(signerInfos.children[0]);
...
function parseSignerInfo(node) {
  ...
  const sid = children[1];
  const issuerRaw = sid.children[0].raw;
  const serialContents = sid.children[1].contents;
  const digestOidHex = children[2].children[0].contents.toString('hex');
  let index = 3;
  let signedAttrs = null;
  if (children[index].tag === Tag.CONTEXT_0) {
    signedAttrs = children[index];
    index += 1;
  }
  index += 1; // signatureAlgorithm — RSA PKCS#1 v1.5 assumed, digest drives the hash
  const signature = children[index].contents;
...
function findSignerCert(cms, embedded) {
...
function verifyCmsSignature(cms, signerCert) {
...
function findMessageDigestAttribute(signedAttrs) {
  for (const attr of signedAttrs.children) {
    if (attr.children[0].contents.equals(OID_MESSAGE_DIGEST)) {
      return attr.children[1].children[0].contents;
    }
  }
  return null;
}
...
function verifyDeviceHash(fields, deviceGuid) {
...
function parsePayload(content) {
  ...
  const fields = {
    bundleId: null, bundleIdBytes: null, appVersion: null, opaqueValue: null,
    sha1Hash: null, creationDate: null, originalAppVersion: null,
    expirationDate: null, inAppPurchases: [],
  };
...
function parseAttributeSet(der, what) {
  let node;
  try {
    node = parse(der);
  } catch (cause) {
  ...
  for (const child of node.children) {
    if (child.tag !== Tag.SEQUENCE || child.children.length < 3
      || child.children[0].tag !== Tag.INTEGER || !isOctetString(child.children[2])) {
      throw new VerificationError(Reason.INVALID_RECEIPT_FORMAT, 'malformed receipt attribute');
    }
    attributes.push({
      type: integerValue(child.children[0]),
      value: octetStringValue(child.children[2]),
    });
  }
  return attributes;
}
...
function decodeNested(der, what) {
  try {
    return parse(der);
  } catch (cause) {
    throw new VerificationError(Reason.INVALID_RECEIPT_FORMAT,
