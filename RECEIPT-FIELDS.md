...
| 2e. Support SHA-256 and SHA-1 signing | `receipt/ReceiptVerifier.java:540-554` accepts digest OIDs `1.3.14.3.2.26` (SHA-1) and `2.16.840.1.101.3.4.2.1` (SHA-256), and nothing else. Certificate signature algorithms are checked by the pinned BouncyCastle provider, so a JVM policy that bans SHA-1 does not reach them; `java/README.md` "One platform caveat worth knowing" states the trade-off. | Matches, stricter (an allow-list of two) |
